...
void g3_get_slew_step(fix step_size, g3s_vector *x_step, g3s_vector *y_step, g3s_vector *z_step) {
    // x vector
    if (x_step) {
        x_step->noname2.gX = fix_mul(step_size, unscaled_matrix.m1);
        x_step->noname2.gY = fix_mul(step_size, unscaled_matrix.m4);
        x_step->noname2.gZ = fix_mul(step_size, unscaled_matrix.m7);
    }

    // y vector
    if (y_step) {
        y_step->noname2.gX = fix_mul(step_size, unscaled_matrix.m2);
        y_step->noname2.gY = fix_mul(step_size, unscaled_matrix.m5);
        y_step->noname2.gZ = fix_mul(step_size, unscaled_matrix.m8);
    }

    // z vector
    if (z_step) {
        z_step->noname2.gX = fix_mul(step_size, unscaled_matrix.m3);
        z_step->noname2.gY = fix_mul(step_size, unscaled_matrix.m6);
        z_step->noname2.gZ = fix_mul(step_size, unscaled_matrix.m9);
    }
}
