...
bool g3_check_poly_facing(g3s_phandle p0, g3s_phandle p1, g3s_phandle p2) {
    ...
    int64_t result =
        fix64_mul(p0->noname2.gX, temp_vector.noname2.gX) + fix64_mul(p0->noname2.gY, temp_vector.noname2.gY) + fix64_mul(p0->noname2.gZ, temp_vector.noname2.gZ);
...
// clang-format off
    ...
    // draw it
    ((void (*)(long c, int n, grs_vertex **vpl))grd_canvas_table[poly_index[(unsigned)gour_flag]])(poly_color, n_verts, p_vpl);
...
{
    if (p0->noname3.rgb != p1->noname3.rgb) {
        gour_flag = 1;
        return (draw_line_common(p0, p1));
    } else {
        gour_flag = 0;
        draw_color = grd_ipal[gr_index_brgb(p0->noname3.rgb)];
        return (draw_line_common(p0, p1));
    }
}
...
#pragma GCC diagnostic pop                  // Restore diagnostics state.
    ...
    } else if (gour_flag > 0) // cline
    {
        ...
        v0.x = p0->sx;
        v0.y = p0->sy;
        gr_split_rgb(p0->noname3.rgb, &a, &b, &c);
        v0.u = a;
        v0.v = b;
        v0.w = c;
        ...
        v1.x = p1->sx;
        v1.y = p1->sy;
        gr_split_rgb(p1->noname3.rgb, &a, &b, &c);
        v1.u = a;
        v1.v = b;
        v1.w = c;
...
bool g3_check_normal_facing(g3s_vector *v, g3s_vector *normal) {
    int64_t result = fix64_mul(v->noname2.gX - _view_position.noname2.gX, normal->noname2.gX) +
        fix64_mul(v->noname2.gY - _view_position.noname2.gY, normal->noname2.gY) +
        fix64_mul(v->noname2.gZ - _view_position.noname2.gZ, normal->noname2.gZ);
