...
#define movsd() *(reinterpret_cast<int*>(edi)) = *(reinterpret_cast<int*>(esi)); esi += 4; edi += 4
...
L0026AA24:
    ...
    eax = *(reinterpret_cast<int*>peax);                                // mov     eax, dword [eax]
    ...
    edx = *(reinterpret_cast<int*>peax);                // mov     edx, dword [eax]
...
static void Draw4x4_InternalAlpha(int* xtab, int b)
{
    ...
    eax = *(reinterpret_cast<int*>(eax));               // mov     eax, dword [eax]
    movsd();                                            // movsd
    movsd();                                            // movsd
L0026AABF:
    // test    edx, edx
    ...
    if (*(reinterpret_cast<int*>(esp+0x4)) > 0x18)      // jg      short L0026AB1B
    {
        goto L0026AB1B;
    }
    goto L0026AB01;                                     // jmp     short L0026AB01
L0026AAD5:
    edi = *(reinterpret_cast<int*>(esp+0x4));           // mov     edi, dword [esp+0x4]
                                                        // cmp     edi, byte 0x0C
    if (edi >= 0x0C) goto L0026AB7A;                    // jge     near L0026AB7A
    ecx = *(reinterpret_cast<int*>(esp));               // mov     ecx, dword [esp]
    ebp = ecx+0x1;                                      // lea     ebp, [ecx+0x1]
    *(reinterpret_cast<int*>(esp)) = ebp;               // mov     dword [esp], ebp
    ecx = *(reinterpret_cast<char*>(ecx));              // mov     cl, byte [ecx]
    ecx &= 0x000000FF;                                  // and     ecx, dword 0x000000FF
    eax <<= 0x08;                                       // shl     eax, 0x08
    eax |= ecx;                                         // or      eax, ecx
    ecx = edi+0x8;                                      // lea     ecx, [edi+0x8]
    *(reinterpret_cast<int*>(esp+0x4)) = ecx;           // mov     dword [esp+0x4], ecx
    goto L0026AAD5;                                     // jmp     short L0026AAD5
L0026AB01:
    edi = esp;                                          // mov     edi, esp
    ecx = *(reinterpret_cast<int*>(edi+0x4));           // mov     ecx, dword [edi+0x4]
    esi = *(reinterpret_cast<int*>(edi));               // mov     esi, dword [edi]
L0026AB08:
    eax <<= 0x08;                                       // shl     eax, 0x08
    eax |= *(reinterpret_cast<uchar*>(esi));            // mov     al, byte [esi]
    esi += 1;                                           // inc     esi
    ecx += 0x08;                                        // add     ecx, byte 0x08
                                                        // cmp     ecx, byte 0x19
    if (ecx < 0x19) goto L0026AB08;                     // jl      short L0026AB08
    *(reinterpret_cast<int*>(edi+0x4)) = ecx;           // mov     dword [edi+0x4], ecx
    *(reinterpret_cast<int*>(edi)) = esi;               // mov     dword [edi], esi
L0026AB1B:
    ecx = *(reinterpret_cast<int*>(esp+0x4));           // mov     ecx, dword [esp+0x4]
    edi = eax;                                          // mov     edi, eax
    ...
    ecx -= edi;                                         // sub     ecx, edi
    ecx += esi;                                         // add     ecx, esi
    esi = *(reinterpret_cast<int*>(ecx));               // mov     esi, dword [ecx]
    edi = esi;                                          // mov     edi, esi
    edi = (uint)edi >> 0x14;                            // shr     edi, 0x14
    edi &= 0x0F;                                        // and     edi, byte 0x0F
                                                        // test    edi, edi
    if (edi == 0) goto L0026AC00;                       // jz      near L0026AC00
    ebp = *(reinterpret_cast<int*>(esp+0x4));           // mov     ebp, dword [esp+0x4]
    ebp -= edi;                                         // sub     ebp, edi
    edi = esi;                                          // mov     edi, esi
    ...
    edx -= 1;                                           // dec     edx
    ecx = edi;                                          // mov     ecx, edi
    *(reinterpret_cast<int*>(esp+0x4)) = ebp;           // mov     dword [esp+0x4], ebp
    ecx &= 0x000E0000;                                  // and     ecx, dword 0x000E0000
    *(reinterpret_cast<int*>(ebx-0x4)) = edi;           // mov     dword [ebx-0x4], edi
                                                        // cmp     ecx, dword 0x000A0000
    if ((uint)(ecx) >= (uint)(0x000A0000))              // jnc     near L0026ABEA
    {
        goto L0026ABEA;
    }
L0026AB7A:
    ...
    ecx += edi;                                         // add     ecx, edi
    edi = *(reinterpret_cast<int*>(ecx));               // mov     edi, dword [ecx]
    esi = edi;                                          // mov     esi, edi
    ...
    *(reinterpret_cast<int*>(ebx-0x4)) = edi;           // mov     dword [ebx-0x4], edi
                                                        // cmp     ecx, dword 0x000A0000
    if ((uint)(ecx) >= (uint)(0x000A0000))              // jnc     short L0026ABEA
    {
        goto L0026ABEA;
    }
    // test    edx, edx
    ...
    if (*(reinterpret_cast<int*>(esp+0x4)) >= 0x0C)     // jge     short L0026AB7A
    {
        goto L0026AB7A;
    }
                                                        // cmp     edx, byte 0x01
...
L0026ABEA:
    ...
    ecx &= 0x000E0000;                                  // and     ecx, dword 0x000E0000
                                                        // cmp     ecx, dword 0x000A0000
    if ((uint)(ecx) < (uint)(0x000A0000))               // jc      near L0026ACE6
    {
        goto L0026ACE6;
    }
...
L0026AC00:
    ...
    *(reinterpret_cast<int*>(esi)) = eax;               // mov     dword [esi], eax
    esi = esp;                                          // mov     esi, esp
    eax = ecx;                                          // mov     eax, ecx
    movsd();                                            // movsd
    movsd();                                            // movsd
    ...
    esi &= 0x0F;                                        // and     esi, byte 0x0F
    edi -= esi;                                         // sub     edi, esi
    *(reinterpret_cast<int*>(esp+0x4)) = edi;           // mov     dword [esp+0x4], edi
    edi = *(reinterpret_cast<int*>(ecx));               // mov     edi, dword [ecx]
    edi &= 0x00FFFFFF;                                  // and     edi, dword 0x00FFFFFF
    ebx += 0x04;                                        // add     ebx, byte 0x04
    ecx = edi;                                          // mov     ecx, edi
    ...
    *(reinterpret_cast<int*>(ebx-0x4)) = edi;           // mov     dword [ebx-0x4], edi
                                                        // cmp     ecx, dword 0x000A0000
    if ((uint)(ecx) >= (uint)(0x000A0000))              // jnc     short L0026AC6A
    {
        goto L0026AC6A;
    }
    // test    edx, edx
...
L0026AC6A:
    ...
    *(reinterpret_cast<int*>(esp)) = esi;               // mov     dword [esp], esi
    ecx = *(reinterpret_cast<char*>(ecx));              // mov     cl, byte [ecx]
    ecx &= 0x000000FF;                                  // and     ecx, dword 0x000000FF
...
L0026ACC3:
    ...
    edx -= ecx;                                         // sub     edx, ecx
    *(reinterpret_cast<int*>(ebx-0x4)) = esi;           // mov     dword [ebx-0x4], esi
    goto L0026AABF;                                     // jmp     near L0026AABF
L0026ACD8:
    ebx += 0x04;                                        // add     ebx, byte 0x04
    ecx = *(reinterpret_cast<int*>(ebx-0x8));           // mov     ecx, dword [ebx-0x8]
    *(reinterpret_cast<int*>(ebx-0x4)) = ecx;           // mov     dword [ebx-0x4], ecx
    goto L0026AABF;                                     // jmp     near L0026AABF
L0026ACE6:
    edx = (int)d4x4_gptr;                               // mov     edx, dword [_d4x4_gptr]
    esi = esp;                                          // mov     esi, esp
    edi = (int)d4x4_eptr;                               // mov     edi, dword [_d4x4_eptr]
    *(reinterpret_cast<int*>(edx)) = eax;               // mov     dword [edx], eax
    movsd();                                            // movsd
    movsd();                                            // movsd
...
static uchar* Draw4x4_InternalBeta(int* xtab, int b, uchar* bits, int d, uchar* mask_stream)
{
    ...
    //ebp = *(reinterpret_cast<int*>(esp+0x48));        // mov     ebp, dword [esp+0x48]
    ebp = (int)mask_stream;
    *(reinterpret_cast<int*>(esp+0x18)) = eax;          // mov     dword [esp+0x18], eax
    *(reinterpret_cast<int*>(esp+0x1C)) = ebx;          // mov     dword [esp+0x1C], ebx
    *(reinterpret_cast<int*>(esp+0x30)) = ecx;          // mov     dword [esp+0x30], ecx
    eax = ecx-0x4;                                      // lea     eax, [ecx-0x4]
    edx <<= 0x02;                                       // shl     edx, 0x02
    *(reinterpret_cast<int*>(esp+0x20)) = eax;          // mov     dword [esp+0x20], eax
    eax = ebx+edx;                                      // lea     eax, [ebx+edx]
    *(reinterpret_cast<int*>(esp+0x10)) = eax;          // mov     dword [esp+0x10], eax
                                                        // cmp     ebx, eax
    if ((uint)(ebx) >= (uint)(eax)) goto L0026B221;     // jnc     near L0026B221
    eax = ecx*4;                                        // lea     eax, [ecx*4]
    eax -= ecx;                                         // sub     eax, ecx
    *(reinterpret_cast<int*>(esp+0xC)) = eax;           // mov     dword [esp+0xC], eax
    eax = ecx+ecx;                                      // lea     eax, [ecx+ecx]
    *(reinterpret_cast<int*>(esp+0x8)) = eax;           // mov     dword [esp+0x8], eax
outer_loop:
    eax = *(reinterpret_cast<int*>(esp+0x18));          // mov     eax, dword [esp+0x18]
    eax = *(reinterpret_cast<int*>(eax));               // mov     eax, dword [eax]
    eax &= 0x0001FFFF;                                  // and     eax, dword 0x0001FFFF
    *(reinterpret_cast<int*>(esp+0x4)) = eax;           // mov     dword [esp+0x4], eax
    eax = *(reinterpret_cast<int*>(esp+0x18));          // mov     eax, dword [esp+0x18]
    eax = *(reinterpret_cast<int*>(eax));               // mov     eax, dword [eax]
    ...
    edx = *(reinterpret_cast<int*>(esp+0x1C));          // mov     edx, dword [esp+0x1C]
    edi = *(reinterpret_cast<int*>(esp+0x8));           // mov     edi, dword [esp+0x8]
    edx += edi;                                         // add     edx, edi
    *(reinterpret_cast<int*>(esp+0x14)) = edx;          // mov     dword [esp+0x14], edx
    edx = ebp+0x4;                                      // lea     edx, [ebp+0x4]
    *(reinterpret_cast<int*>(esp)) = edx;               // mov     dword [esp], edx
...
L0026AD9F:
    eax = *(reinterpret_cast<int*>(esp+0x4));           // mov     eax, dword [esp+0x4]
    esi = *(reinterpret_cast<int*>(esp+0x4));           // mov     esi, dword [esp+0x4]
    eax <<= 0x10;                                       // shl     eax, 0x10
    edx = *(reinterpret_cast<int*>(esp+0x1C));          // mov     edx, dword [esp+0x1C]
    eax |= esi;                                         // or      eax, esi
    edi = *(reinterpret_cast<int*>(esp+0x30));          // mov     edi, dword [esp+0x30]
    *(reinterpret_cast<int*>(edx)) = eax;               // mov     dword [edx], eax
    edx += edi;                                         // add     edx, edi
    *(reinterpret_cast<int*>(edx)) = eax;               // mov     dword [edx], eax
    edx = *(reinterpret_cast<int*>(esp+0x14));          // mov     edx, dword [esp+0x14]
    ebx = *(reinterpret_cast<int*>(esp+0xC));           // mov     ebx, dword [esp+0xC]
    *(reinterpret_cast<int*>(edx)) = eax;               // mov     dword [edx], eax
    edx = *(reinterpret_cast<int*>(esp+0x1C));          // mov     edx, dword [esp+0x1C]
    edx += ebx;                                         // add     edx, ebx
    *(reinterpret_cast<int*>(edx)) = eax;                               // mov     dword [edx], eax
...
L0026ADD1:
    eax = *(reinterpret_cast<int*>(esp+0x1C));          // mov     eax, dword [esp+0x1C]
    esi = esp+0x4;                                      // lea     esi, [esp+0x4]
    ebp += 0x02;                                        // add     ebp, byte 0x02
    edx = 0;                                            // xor     edx, edx
    //ch = *(reinterpret_cast<char*>(esp+0x4));         // mov     ch, byte [esp+0x4]
    edx = *(reinterpret_cast<ushort*>(ebp-0x2));        // mov     dx, word [ebp-0x2]
                                                        // test    ch, ch
    if (*(reinterpret_cast<char*>(esp+0x4)) == 0)       // jz      short L0026AE34
    {
        goto L0026AE34;
    }
    ebx = *(reinterpret_cast<int*>(esp+0x30));          // mov     ebx, dword [esp+0x30]
    edi = 0;                                            // xor     edi, edi
L0026ADF0:
    ecx = edx;                                          // mov     ecx, edx
    ecx &= 0x01;                                        // and     ecx, byte 0x01
    cl0 = *(reinterpret_cast<char*>(ecx+esi));          // mov     cl, byte [ecx+esi]
    *(reinterpret_cast<char*>(eax)) = cl0;              // mov     byte [eax], cl
    ecx = edx;                                          // mov     ecx, edx
    asm_shr(ecx, 0x00000001);                           // shr     ecx, 0x00000001
    ecx &= 0x01;                                        // and     ecx, byte 0x01
    cl0 = *(reinterpret_cast<char*>(ecx+esi));          // mov     cl, byte [ecx+esi]
    *(reinterpret_cast<char*>(eax+0x1)) = cl0;          // mov     byte [eax+0x1], cl
    ecx = edx;                                          // mov     ecx, edx
    asm_shr(ecx, 0x02);                                 // shr     ecx, 0x02
    ecx &= 0x01;                                        // and     ecx, byte 0x01
    cl0 = *(reinterpret_cast<char*>(ecx+esi));          // mov     cl, byte [ecx+esi]
    *(reinterpret_cast<char*>(eax+0x2)) = cl0;          // mov     byte [eax+0x2], cl
    ecx = edx;                                          // mov     ecx, edx
    ...
    *(reinterpret_cast<char*>(eax+0x3)) = cl0;          // mov     byte [eax+0x3], cl
    eax += ebx;                                         // add     eax, ebx
                                                        // cmp     edi, byte 0x04
...
L0026AE34:
    ecx = 0;                                            // xor     ecx, ecx
    ecx = *(reinterpret_cast<uchar*>(esp+0x5));         // mov     cl, byte [esp+0x5]
    ebx = ecx;                                          // mov     ebx, ecx
    edi = 0x00000004;                                   // mov     edi, dword 0x00000004
    ebx <<= 0x08;                                       // shl     ebx, 0x08
    esi = *(reinterpret_cast<int*>(esp+0x30));          // mov     esi, dword [esp+0x30]
    ecx |= ebx;                                         // or      ecx, ebx
L0026AE4A:
    // test    dl, byte 0x03
    ...
    *(reinterpret_cast<ushort*>(eax)) = (ushort)ecx;    // mov     word [eax], cx
    goto L0026AE65;                                     // jmp     short L0026AE65
L0026AE5E:
    *(reinterpret_cast<uchar*>(eax)) = (uchar)ecx;      // mov     byte [eax], cl
    goto L0026AE65;                                     // jmp     short L0026AE65
L0026AE62:
    *(reinterpret_cast<uchar*>(eax+0x1)) = (uchar)ecx;  // mov     byte [eax+0x1], cl
L0026AE65:
    // test    dl, byte 0x0C
    ...
    *(reinterpret_cast<ushort*>(eax+0x2)) = (ushort)ecx;// mov     word [eax+0x2], cx
    goto L0026AE82;                                     // jmp     short L0026AE82
L0026AE7A:
    *(reinterpret_cast<uchar*>(eax+0x2)) = (uchar)ecx;  // mov     byte [eax+0x2], cl
    goto L0026AE82;                                     // jmp     short L0026AE82
L0026AE7F:
    *(reinterpret_cast<uchar*>(eax+0x3)) = (uchar)ecx;  // mov     byte [eax+0x3], cl
L0026AE82:
    asm_shr(edx, 0x04);                                 // shr     edx, 0x04
    edi -= 1;                                           // dec     edi
    eax += esi;                                         // add     eax, esi
...
L0026AE92:
    eax = *(reinterpret_cast<int*>(ebp));               // mov     eax, dword [ebp]
    *(reinterpret_cast<int*>(esp+0x34)) = eax;          // mov     dword [esp+0x34], eax
    ebp = edx;                                          // mov     ebp, edx
    edx = *(reinterpret_cast<int*>(esp+0x4));           // mov     edx, dword [esp+0x4]
    eax = (int)d4x4_colorset;                           // mov     eax, dword [_d4x4_colorset]
    edx += eax;                                         // add     edx, eax
    edi = *(reinterpret_cast<int*>(esp+0x1C));          // mov     edi, dword [esp+0x1C]
    //cl = *(reinterpret_cast<char*>(edx));             // mov     cl, byte [edx]
    *(reinterpret_cast<int*>(esp+0x24)) = edx;          // mov     dword [esp+0x24], edx
                                                        // test    cl, cl
    if (*(reinterpret_cast<char*>(edx)) == 0)           // jz      near L0026AF6A
    {
        goto L0026AF6A;
    }
    *(reinterpret_cast<int*>(esp+0x28)) = 0x00000004;   // mov     dword [esp+0x28], dword 0x00000004
L0026AEC0:
    esi = *(reinterpret_cast<int*>(esp+0x24));          // mov     esi, dword [esp+0x24]
    edx = *(reinterpret_cast<int*>(esp+0x34));          // mov     edx, dword [esp+0x34]
    ecx = edi;                                          // mov     ecx, edi
    eax = edx;                                          // mov     eax, edx
    eax &= 0x03;                                        // and     eax, byte 0x03
    ebx &= 0xFFFF0000;
    ebx |= *(reinterpret_cast<uchar*>(esi+eax));        // mov     bl, byte [esi+eax]
    eax = edx;                                          // mov     eax, edx
    asm_shr(eax, 0x02);                                 // shr     eax, 0x02
    eax &= 0x03;                                        // and     eax, byte 0x03
    ebx |= *(reinterpret_cast<uchar*>(esi+eax)) << 8;                   // mov     bh, byte [esi+eax]
    *(reinterpret_cast<ushort*>(ecx)) = ebx & 0xFFFF;                   // mov     word [ecx], bx
    eax = edx;                                          // mov     eax, edx
    ...
    eax &= 0x03;                                        // and     eax, byte 0x03
    ebx |= *(reinterpret_cast<uchar*>(esi+eax)) << 8;   // mov     bh, byte [esi+eax]
    *(reinterpret_cast<ushort*>(ecx+0x2)) = ebx & 0xFFFF;  // mov     word [ecx+0x2], bx
    ecx = edx;                                          // mov     ecx, edx
    esi = *(reinterpret_cast<int*>(esp+0x30));          // mov     esi, dword [esp+0x30]
    asm_shr(ecx, 0x08);                                 // shr     ecx, 0x08
    edi += esi;                                         // add     edi, esi
    *(reinterpret_cast<int*>(esp+0x34)) = ecx;          // mov     dword [esp+0x34], ecx
    esi = *(reinterpret_cast<int*>(esp+0x24));          // mov     esi, dword [esp+0x24]
    edx = *(reinterpret_cast<int*>(esp+0x34));          // mov     edx, dword [esp+0x34]
    ecx = edi;                                          // mov     ecx, edi
    eax = edx;                                          // mov     eax, edx
    ...
    *(reinterpret_cast<ushort*>(ecx)) = ebx & 0xFFFF;   // mov     word [ecx], bx
    ...
    ebx |= *(reinterpret_cast<uchar*>(esi+eax)) << 8;   // mov     bh, byte [esi+eax]
    *(reinterpret_cast<ushort*>(ecx+0x2)) = ebx & 0xFFFF;  // mov     word [ecx+0x2], bx
    ebx = *(reinterpret_cast<int*>(esp+0x28));          // mov     ebx, dword [esp+0x28]
    eax = edx;                                          // mov     eax, edx
    edx = *(reinterpret_cast<int*>(esp+0x30));          // mov     edx, dword [esp+0x30]
    asm_shr(eax, 0x08);                                 // shr     eax, 0x08
    ebx -= 0x02;                                        // sub     ebx, byte 0x02
    *(reinterpret_cast<int*>(esp+0x34)) = eax;          // mov     dword [esp+0x34], eax
    *(reinterpret_cast<int*>(esp+0x28)) = ebx;          // mov     dword [esp+0x28], ebx
    edi += edx;                                         // add     edi, edx
                                                        // test    ebx, ebx
    if (ebx <= 0) goto L0026B1FF;                       // jle     near L0026B1FF
    goto L0026AEC0;                                     // jmp     near L0026AEC0
L0026AF6A:
    ebx = *(reinterpret_cast<int*>(esp+0x34));          // mov     ebx, dword [esp+0x34]
    esi = edx;                                          // mov     esi, edx
    edx = *(reinterpret_cast<int*>(esp+0x30));          // mov     edx, dword [esp+0x30]
...
simple_loop:
    ...
    al0 = *(reinterpret_cast<char*>(esi+ecx));          // mov     al, byte [esi+ecx]
    *(reinterpret_cast<char*>(edi)) = al0;              // mov     byte [edi], al
L0026AF82:
    ecx = ebx;                                          // mov     ecx, ebx
    ...
    asm_shr(ecx, 0x02);                                 // shr     ecx, 0x02
    al0 = *(reinterpret_cast<char*>(esi+ecx));          // mov     al, byte [esi+ecx]
    *(reinterpret_cast<char*>(edi+0x1)) = al0;          // mov     byte [edi+0x1], al
L0026AF92:
    ecx = ebx;                                          // mov     ecx, ebx
    ...
    al0 = *(reinterpret_cast<char*>(esi+ecx));          // mov     al, byte [esi+ecx]
    *(reinterpret_cast<char*>(edi+0x2)) = al0;          // mov     byte [edi+0x2], al
L0026AFA2:
    ecx = ebx;                                          // mov     ecx, ebx
    ...
    al0 = *(reinterpret_cast<char*>(esi+ecx));          // mov     al, byte [esi+ecx]
    *(reinterpret_cast<char*>(edi+0x3)) = al0;          // mov     byte [edi+0x3], al
L0026AFB5:
    edi += edx;                                         // add     edi, edx
    ...
    goto L0026B1FF;                                     // jmp     near L0026B1FF
L0026AFC3:
    ebx = *(reinterpret_cast<int*>(esp+0x4));           // mov     ebx, dword [esp+0x4]
    esi = (int)d4x4_colorset;                           // mov     esi, dword [_d4x4_colorset]
    esi += ebx;                                         // add     esi, ebx
    edx = *(reinterpret_cast<int*>(esp+0x1C));          // mov     edx, dword [esp+0x1C]
    eax = *(reinterpret_cast<int*>(ebp));               // mov     eax, dword [ebp]
    //bh = *(reinterpret_cast<char*>(esi));             // mov     bh, byte [esi]
    ebp = *(reinterpret_cast<int*>(esp));               // mov     ebp, dword [esp]
                                                        // test    bh, bh
    if (*(reinterpret_cast<char*>(esi)) == 0)           // jz      short L0026B03D
    {
        goto L0026B03D;
    }
    ebx = *(reinterpret_cast<int*>(esp+0x30));          // mov     ebx, dword [esp+0x30]
...
L0026AFE5:
    ...
    cl0 = *(reinterpret_cast<char*>(ecx+esi));          // mov     cl, byte [ecx+esi]
    *(reinterpret_cast<char*>(edx)) = cl0;              // mov     byte [edx], cl
    ecx = eax;                                          // mov     ecx, eax
    asm_shr(ecx, 0x03);                                 // shr     ecx, 0x03
    ecx &= 0x07;                                        // and     ecx, byte 0x07
    cl0 = *(reinterpret_cast<char*>(ecx+esi));          // mov     cl, byte [ecx+esi]
    *(reinterpret_cast<char*>(edx+0x1)) = cl0;          // mov     byte [edx+0x1], cl
    ecx = eax;                                          // mov     ecx, eax
    asm_shr(ecx, 0x06);                                 // shr     ecx, 0x06
    ecx &= 0x07;                                        // and     ecx, byte 0x07
    cl0 = *(reinterpret_cast<char*>(ecx+esi));          // mov     cl, byte [ecx+esi]
    *(reinterpret_cast<char*>(edx+0x2)) = cl0;          // mov     byte [edx+0x2], cl
    ecx = eax;                                          // mov     ecx, eax
    asm_shr(ecx, 0x09);                                 // shr     ecx, 0x09
    ecx &= 0x07;                                        // and     ecx, byte 0x07
    cl0 = *(reinterpret_cast<char*>(ecx+esi));          // mov     cl, byte [ecx+esi]
    asm_shr(eax, 0x0C);                                 // shr     eax, 0x0C
    *(reinterpret_cast<char*>(edx+0x3)) = cl0;          // mov     byte [edx+0x3], cl
    edx += ebx;                                         // add     edx, ebx
                                                        // cmp     edi, byte 0x01
    if (edi != 0x01) goto L0026B031;                    // jnz     short L0026B031
    ecx = 0;                                            // xor     ecx, ecx
    ecx = *(reinterpret_cast<ushort*>(ebp));            // mov     cx, word [ebp]
...
L0026B03D:
    edx = 0;                                            // xor     edx, edx
    edi = *(reinterpret_cast<int*>(esp+0x1C));          // mov     edi, dword [esp+0x1C]
    edx = *(reinterpret_cast<ushort*>(ebp));            // mov     dx, word [ebp]
    ebx = eax;                                          // mov     ebx, eax
    eax = edx;                                          // mov     eax, edx
    edx = *(reinterpret_cast<int*>(esp+0x30));          // mov     edx, dword [esp+0x30]
    ebp += 0x02;                                        // add     ebp, byte 0x02
...
L0026B05C:
    ...
    cl0 = *(reinterpret_cast<char*>(esi+ecx));                          // mov     cl, byte [esi+ecx]
    *(reinterpret_cast<char*>(edi)) = cl0;                              // mov     byte [edi], cl
L0026B068:
    ecx = ebx;                                          // mov     ecx, ebx
    ...
    asm_shr(ecx, 0x03);                                 // shr     ecx, 0x03
    cl0 = *(reinterpret_cast<char*>(esi+ecx));          // mov     cl, byte [esi+ecx]
    *(reinterpret_cast<char*>(edi+0x1)) = cl0;          // mov     byte [edi+0x1], cl
L0026B078:
    ecx = ebx;                                          // mov     ecx, ebx
    ...
    asm_shr(ecx, 0x06);                                 // shr     ecx, 0x06
    cl0 = *(reinterpret_cast<char*>(esi+ecx));                          // mov     cl, byte [esi+ecx]
    *(reinterpret_cast<char*>(edi+0x2)) = cl0;                          // mov     byte [edi+0x2], cl
L0026B08B:
    ecx = ebx;                                          // mov     ecx, ebx
    ...
    asm_shr(ecx, 0x09);                                 // shr     ecx, 0x09
    cl0 = *(reinterpret_cast<char*>(esi+ecx));          // mov     cl, byte [esi+ecx]
    *(reinterpret_cast<char*>(edi+0x3)) = cl0;          // mov     byte [edi+0x3], cl
L0026B09E:
    edi += edx;                                         // add     edi, edx
...
L0026B0BE:
    ...
    ebx += edx;                                         // add     ebx, edx
    *(reinterpret_cast<int*>(esp+0x2C)) = ebx;          // mov     dword [esp+0x2C], ebx
    bl0 = *(reinterpret_cast<char*>(ebx));              // mov     bl, byte [ebx]
    eax = *(reinterpret_cast<int*>(esp+0x1C));          // mov     eax, dword [esp+0x1C]
                                                        // test    bl, bl
...
L0026B0DF:
    edx = 0;                                            // xor     edx, edx
    edx = *(reinterpret_cast<uchar*>(ebp));             // mov     dl, byte [ebp]
    ecx = edx;                                          // mov     ecx, edx
    esi = *(reinterpret_cast<int*>(esp+0x2C));          // mov     esi, dword [esp+0x2C]
    ecx &= ebx;                                         // and     ecx, ebx
    ecx += esi;                                         // add     ecx, esi
    asm_shr(edx, 0x04);                                 // shr     edx, 0x04
    cl0 = *(reinterpret_cast<char*>(ecx));              // mov     cl, byte [ecx]
    edx += esi;                                         // add     edx, esi
    *(reinterpret_cast<char*>(eax)) = cl0;              // mov     byte [eax], cl
    eax += 1;                                           // inc     eax
    dl0 = *(reinterpret_cast<char*>(edx));              // mov     dl, byte [edx]
    ebp += 1;                                           // inc     ebp
    *(reinterpret_cast<char*>(eax)) = dl0;              // mov     byte [eax], dl
    edx = 0;                                            // xor     edx, edx
    edi += 1;                                           // inc     edi
    edx = *(reinterpret_cast<uchar*>(ebp));             // mov     dl, byte [ebp]
    eax += 1;                                           // inc     eax
    ...
    *(reinterpret_cast<char*>(eax-0x1)) = cl0;          // mov     byte [eax-0x1], cl
    eax += 1;                                           // inc     eax
    dl0 = *(reinterpret_cast<char*>(edx));              // mov     dl, byte [edx]
    esi = *(reinterpret_cast<int*>(esp+0x20));          // mov     esi, dword [esp+0x20]
    *(reinterpret_cast<char*>(eax-0x1)) = dl0;          // mov     byte [eax-0x1], dl
    eax += esi;                                         // add     eax, esi
                                                        // cmp     edi, byte 0x04
    if (edi >= 0x04) goto L0026B1FF;                    // jge     near L0026B1FF
...
L0026B12D:
    ...
    edx = *(reinterpret_cast<int*>(esp+0x30));          // mov     edx, dword [esp+0x30]
    edi = eax;                                          // mov     edi, eax
    ebx = *(reinterpret_cast<int*>(ebp));               // mov     ebx, dword [ebp]
    eax = 0x00000002;                                   // mov     eax, dword 0x00000002
L0026B13F:
    ecx = ebx;                                          // mov     ecx, ebx
    ecx &= 0x0F;                                        // and     ecx, byte 0x0F
...
L0026B184:
    ...
    edi = *(reinterpret_cast<int*>(esp+0x14));                          // mov     edi, dword [esp+0x14]
    edx = *(reinterpret_cast<int*>(esp+0x30));                          // mov     edx, dword [esp+0x30]
    ebx = *(reinterpret_cast<int*>(ebp+0x4));                           // mov     ebx, dword [ebp+0x4]
...
L0026B1EE:
    ...
    edx += eax;                                         // add     edx, eax
    *(reinterpret_cast<int*>(esp+0x1C)) = edx;          // mov     dword [esp+0x1C], edx
L0026B1FF:
    ebx = *(reinterpret_cast<int*>(esp+0x18));          // mov     ebx, dword [esp+0x18]
    ecx = *(reinterpret_cast<int*>(esp+0x1C));          // mov     ecx, dword [esp+0x1C]
    esi = *(reinterpret_cast<int*>(esp+0x10));          // mov     esi, dword [esp+0x10]
    ebx += 0x04;                                        // add     ebx, byte 0x04
    ecx += 0x04;                                        // add     ecx, byte 0x04
    *(reinterpret_cast<int*>(esp+0x18)) = ebx;                          // mov     dword [esp+0x18], ebx
    *(reinterpret_cast<int*>(esp+0x18)) = ebx;          // mov     dword [esp+0x1C], ecx
                                                        // cmp     ecx, esi
...
L0026B221:
    ...
    uchar *retval;
    memcpy(retval, &eax, sizeof(uchar));
