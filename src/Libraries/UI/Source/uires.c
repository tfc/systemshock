...
//      Warning(("Bitmap index %i invalid!\n",i));
    ...
    rid = MKREF(id_num,i);
    if (uiResTempBuffer.mem == NULL || (unsigned)RefSize(rt,i) > uiResTempBuffer.size)
    {
...
//   extern int memcount;
    ...
    RefTable *rt = NULL;
    if (uiResTempBuffer.mem != NULL && (unsigned)tsize <= uiResTempBuffer.size)
    {
