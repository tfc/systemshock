...
int lg_vsprintf(char *buf, const char *format, va_list arglist)
{
    ...
    uchar ladjust = 0, altform = 0, pspec = 0, this_is_len;
    bigness big;
    char pad_char = 0;
    char src_char;
    int dest_ind, src_ind, len, int_part, frac_part, mult;
    int fwid = 0, precis = 0, newchars = 0, fshift, shift_ind, prefix;
    lgsStage stage;
        ...
        if( stage!=STAGE_TEXT ) {
            switch( src_char ) {
              ...
              case 'b': // bool
                  ...
                  arg_bool=!!va_arg(arglist,int); // Not sure if this is correct.
                  if(altform) {
