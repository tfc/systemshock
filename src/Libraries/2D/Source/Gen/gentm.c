...
/*{
  ...
  DebugString(s);
    ...
    info.noname8.left_edge_func=info.noname9.right_edge_func=info.loop_func=gr_null;
...
// moved this code from the #define in PolyInt.h because the compiler couldn't handle it
    ...
    info.noname4.bm = *bm;       //  memcpy(&(info.bm),bm,sizeof(*bm));
    info.noname1.y=y_min;
    info.u_mask=(1<<bm->wlog)-1;
    info.noname7.v_mask=((1<<bm->hlog)-1)<<bm->wlog;
    info.vtab=NULL;
    ...
    for (y=y_min; (unsigned)y!=y_max; ) {
        ...
        if (fix_cint((*p_left)->y)<=y) {
            ...
            ((edge_type) info.noname8.left_edge_func) (&info,p_left,&prev,TMS_LEFT);
        }
        ...
        if (fix_cint((*p_right)->y)<=y) {
            ...
            ((edge_type) info.noname9.right_edge_func) (&info,p_right,&prev,TMS_RIGHT);
        }
        y_limit=lg_min(info.noname6.right.y,info.noname5.left.y);
        info.n=y_limit-y;
...
void v_umap(grs_bitmap *bm, int n, grs_vertex **vpl, grs_tmap_info *ti)
{
    ...
    info.noname8.top_edge_func=info.noname9.bot_edge_func=info.loop_func=gr_null;
    ...
    for (x=x_min; (unsigned)x!=x_max; ) {
        ...
        if (fix_cint((*p_top)->x)<=x) {
            ...
            ((edge_type) info.noname8.top_edge_func) (&info,p_top,&prev,TMS_LEFT);
        }
        ...
        if (fix_cint((*p_bot)->x)<=x) {
            ...
            ((edge_type) info.noname9.bot_edge_func) (&info,p_bot,&prev,TMS_RIGHT);
        }
        x_limit=lg_min(info.noname6.bot.x,info.noname5.top.x);
        info.n=x_limit-x;
