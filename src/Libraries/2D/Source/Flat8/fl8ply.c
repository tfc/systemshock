...
int gri_poly_loop(grs_tmap_loop_info *ti) {
    int d;
    uchar c = (uchar)(intptr_t)(ti->noname4.bm.bits); /* actually, fill_parm */
    uchar *bm_bits = ti->noname4.bm.bits;
    uchar *ti_d = ti->noname1.d;
    fix ti_left_x = ti->noname5.left.x;
    fix ti_right_x = ti->noname6.right.x;
    uchar *ti_clut = ti->clut;
    uchar ti_hlog = ti->noname4.bm.hlog;
    ushort grow = grd_bm.row;
    fix ti_left_dx = ti->noname5.left.noname.dx;
    fix ti_right_dx = ti->noname6.right.noname.dx;
    ...
    ti->noname1.d = ti_d;
    ti->noname6.right.x = ti_right_x;
    ti->noname5.left.x = ti_left_x;
    return FALSE;
}

void gri_solid_poly_init(grs_tmap_loop_info *ti) {
    ti->noname4.bm.bits = ti->clut; /* set fill_parm */
    ti->noname4.bm.hlog = GRL_OPAQUE;
    ti->noname1.d = ti->noname1.y * grd_bm.row + grd_bm.bits;
    ti->loop_func = (void (*)())gri_poly_loop;
    ti->noname8.top_edge_func = (void (*)())gri_x_edge;
    ti->noname9.bot_edge_func = (void (*)())gri_x_edge;
}
...
void gri_clut_poly_init(grs_tmap_loop_info *ti) {
    ti->noname4.bm.bits = (uchar *)(intptr_t)ti->clut[(intptr_t)ti->noname4.bm.bits];
    ti->noname4.bm.hlog = GRL_OPAQUE;
    ti->noname1.d = ti->noname1.y * grd_bm.row + grd_bm.bits;
    ti->loop_func = (void (*)())gri_poly_loop;
    ti->noname8.top_edge_func = (void (*)())gri_x_edge;
...
void gri_tpoly_init(grs_tmap_loop_info *ti) {
    if (tluc8tab[(intptr_t)(ti->noname4.bm.bits)] != NULL) {
        ti->noname4.bm.bits = tluc8tab[(intptr_t)ti->noname4.bm.bits];
...
void gri_clut_tpoly_init(grs_tmap_loop_info *ti) {
    ...
    if (tluc8tab[(intptr_t)(ti->noname4.bm.bits)] != NULL) {
        ti->noname4.bm.bits = tluc8tab[(intptr_t)ti->noname4.bm.bits];
        ti->noname4.bm.hlog = GRL_TLUC8 | GRL_CLUT;
    } else {
        ti->noname4.bm.bits = (uchar *)(intptr_t)ti->clut[(intptr_t)ti->noname4.bm.bits];
        ti->noname4.bm.hlog = GRL_OPAQUE;
    }
