...
{
    uint head = 0;
    while (head < (unsigned)q->fullness)
    {
        uint lchild = LCHILD(head);
        uint rchild = RCHILD(head);
        uint minchild = NULL_CHILD;
        if (rchild >= (unsigned)q->fullness)
        {
            minchild = lchild;
        }
        if (lchild >= (unsigned)q->fullness)
        {
            minchild = rchild;
        }
        ...
        if (minchild < (unsigned)q->fullness && LESS(q,minchild,head))
        {
...
void double_re_heapify(PQueue *q, int head)
{
    ...
    if (minchild < (unsigned)q->fullness && LESS(q,minchild,head))
    {
        ...
        if (maxchild < (unsigned)q->fullness)
        {
...
errtype pqueue_read(PQueue* q, FILE * fd, void (*readfunc)(int fd, void* elem))
{
    int retval;
    int i;
    retval = fread((char*)q,1,sizeof(PQueue), fd);
    ...
    for(i = 0; i < q->fullness; i++)
    {
        ...
        {
            //readfunc(fd,NTH(q,i));
            readfunc(*(int *)fd,NTH(q,i));
        }
        else
        {
            retval = fread((char*)NTH(q,i),1,q->elemsize, fd);
        }
    }
    (void)retval;
    return OK;
